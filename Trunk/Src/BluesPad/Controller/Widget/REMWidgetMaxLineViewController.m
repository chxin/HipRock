//
//  REMWidgetMaxLineViewController.m
//  Blues
//
...
@interface REMWidgetMaxLineViewController ()
@property  (nonatomic) NSNumber *currentSelectedIndex;
@property  (nonatomic,strong) NSMutableArray *datasource;
@end

@implementation REMWidgetMaxLineViewController
...
- (void)initData
{
    ...
    if(amountOfSeries == 1)
    {
        REMTargetEnergyData* seriesData = self.data.targetGlobalData;
        
        NSString* targetIdentity = [NSString stringWithFormat:@"%d-%d-%llu", 0, seriesData.target.type, seriesData.target.targetId];
...
- (void)initLineChart
{
    ...
    self.graph = [[CPTXYGraph alloc]initWithFrame:hostView.frame];
    ...
    double xAxisStart = 0, xAxisLength = 0, globalStart=0, globalLength = 0;
    if(self.data.targetEnergyData.count == 1)
    {
        REMTimeRange *visableTimeRange = self.widgetModel.contentSyntax.timeRanges[0];//self.data.targetGlobalData.target.timeRange;
        
        xAxisStart = [visableTimeRange.startTime timeIntervalSince1970];
        xAxisLength = [visableTimeRange.endTime timeIntervalSince1970]-[visableTimeRange.startTime timeIntervalSince1970];
        
        REMTimeRange *globalTimeRange = self.data.targetGlobalData.target.globalTimeRange;
        
        globalStart = [globalTimeRange.startTime timeIntervalSince1970];
        globalLength = [globalTimeRange.endTime timeIntervalSince1970] - [globalTimeRange.startTime timeIntervalSince1970];
    }
    
    [REMWidgetAxisHelper decorateAxisSet:graph dataSource:self.datasource xStart:xAxisStart xLength:xAxisLength globalStart:globalStart globalRange:globalLength yStartZero:NO];
    //[REMWidgetAxisHelper decorateAxisSet:graph dataSource:self.datasource startPointIndex:xAxisStart endPointIndex:xAxisLength];
...
-(void)scatterPlot:(CPTScatterPlot *)plot plotSymbolWasSelectedAtRecordIndex:(NSUInteger)idx withEvent:(CPTNativeEvent *)event
{
    ...
    NSDictionary* pointData = [[[self.datasource objectAtIndex:[index integerValue]] objectForKey:@"data"] objectAtIndex:idx];
    NSString* xLabelOfPoint = [(NSDateFormatter*)(((CPTXYAxisSet*)(plot.graph.axisSet)).xAxis.labelFormatter) stringFromDate: [pointData objectForKey:@"x"]] ;
