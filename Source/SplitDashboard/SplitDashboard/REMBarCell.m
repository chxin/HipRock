...
- (void)initBarData
{
    ...
    for(int i=1;i<=3;i++)
    {
        ...
        for(int j=1;j<=500;j++)
        {
            ...
            NSNumber *yValue=[NSNumber numberWithInt:235*(i%2)+i*10];
...
- (void) loadBarChart
{
    ...
    plotSpace.xRange= [CPTPlotRange plotRangeWithLocation:CPTDecimalFromLong(0) length:CPTDecimalFromInt(10)];
    ...
    {
        //CPTBarPlot *line = [ CPTBarPlot tubularBarPlotWithColor:[self getColor:idx] horizontalBars:NO];
        ...
        line.barWidth=CPTDecimalFromFloat(3);
        line.borderWidth=0;
        line.fill= [CPTFill fillWithColor:[self getColor:idx]];
        NSLog(@"line.id%@",line.identifier);
        line.barOffset=CPTDecimalFromFloat(offset);
        line.borderColor=nil;
        line.barWidthsAreInViewCoordinates=YES;
        line.lineStyle=nil;
        line.baseValue=CPTDecimalFromFloat(0);
        line.delegate=self;
        line.shadow=nil;
...
- (NSUInteger)numberOfRecordsForPlot:(CPTPlot *)plot
{
    ...
    REMSeries *series= (REMSeries *)self.data.series[0];
    NSLog(@"bar data count:%d",series.data.count);
